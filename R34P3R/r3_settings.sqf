...
//  GLOBAL SETTINGS
r3_autoSelectLeader = true;               // Player is always Leader in SP and in MP if he plays alone.

r3_enableReviveScript = true;             // enable Revive Global
r3_enableAiDogs = false;                  // enable AI Dogs
r3_enableWeatherScript = true;            // enable weather script
...
r3_enableDebugMode = false;               // enable Debug Mode = FALSE (This will flood your LOG FILE !)

r3_moveJIPtoLeader = true;				  // JIP Player will moved to the leader unit
r3_moveJIPafterSec = 10;				  // Move JIP after time (Sec)
r3_moveJIPdistance = 100;				  // Move JIP only if distance to leader bigger then this value

r3_enableEarPlugsOnVehicles = false;        // player can use Ear plugs on Vehicles
r3_savePlayerLoadoutForRespawn = false;     // player respawning with last known loadout

r3_allowColorCorrection = true;            // Allow color correction (Day and Night) (global)
r3_allowSpecialIcons = true;       		   // Allow special icons and player names (global)
r3_SpecialIconsMaxDistance = 200;     	   // maximum distance the icons will shown
r3_allowStaminaBar = true;				   // Allow ShackTac Stamina Bar (global)
r3_allowStaminaScript = true;              // Allow players to enable stamina script
r3_allowAutoHeal = true;       			   // Allow players to enable AutoHeal
r3_allowFSNVG = true;					   // Allow full Screen NightVision

//
//  TIME SETTINGS
//
r3_nightStart = 19;  // 19.5 = 19:30 CET
r3_dayStart = 4.5;     // 04.7 = 04:45 CET
...
//
r3_weatherGlobalSetting = "MIN";     // values: RANDOM, MIN, MAX (r3_weatherParam can be used as override from parameters)
r3_weatherChangeTime = 900;          // Seconds | 900 = 15 minutes
r3_weatherOvercast_min = 0.33;       // minimum overcast value, 0 to 1
r3_weatherOvercast_max = 0.75;       // max overcast value, 0 to 1
r3_weatherRain_min = 0.1;            // minimum rain value, 0 to 1
r3_weatherRain_max = 0.6;            // max rain value, 0 to 1
r3_weatherWind_min = 1;              // minimum wind value, 1 is good 
r3_weatherWind_max = 5;              // max wind value, 15 seems to be max
